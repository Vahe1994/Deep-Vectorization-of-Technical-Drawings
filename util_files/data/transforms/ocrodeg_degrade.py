...
def bounded_gaussian_noise(shape, sigma, maxdelta):
    n, m = shape
    deltas = np.random.random_sample(size=(2, n, m))
...
def make_noise_at_scale(shape, scale):
    ...
    data = np.random.random_sample(size=(h0, w0))
...
def random_blobs(shape, blobdensity, size, roughness=2.0):
    ...
    mask -= np.amin(mask)
    mask /= np.amax(mask)
    noise = np.random.random_sample(size=(h, w))
...
def printlike_fibrous(image, blur=1.0, blotches=5e-5):
    ...
    paper -= make_fibrous_image(image.shape, 300, 500, 0.01, frange=(0.0, 0.25), blur=0.5)
